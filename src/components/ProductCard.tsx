...
const ProductCard: React.FC<Product> = (product) => {
  ...
  return (
    <Link to={`/product/${product.id}`} className="group relative bg-white shadow-sm hover:shadow-lg transition-all duration-500 overflow-hidden w-full max-w-xs mx-auto block">
      {/* Product Image */}
        ...
        >
          <Heart 
            className={`w-3.5 h-3.5 transition-colors ${
              inWishlist ? 'text-rose-500 fill-rose-500' : 'text-gray-600 hover:text-rose-500'
            }`} 
          />
        </button>
